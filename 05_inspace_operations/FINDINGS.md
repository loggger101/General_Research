...
## Round-9 status (domain 5)
Every physical row in `operational_costs.csv` now has either an anchored source or is explicitly model-internal bookkeeping — with one partial exception, named here rather than papered over. Power chains = nextc_ppu_2020 / nextc_protoflight_2021 / ambrosi_2019 / hoffman_2000 (rounds 4–7); storage = ali_2023 (this round); ISRU/volatile-recovery = kleinhenz_2016/2018 + carlson_2024; beneficiation yield per type = SSAP table (round-1). **`Drilling / excavation energy` is only PARTIALLY anchored**: its current citation ("Zacny et al. NIAC studies") has a verifiable NTRS handle via Kleinhenz's co-authorship, but no full text of the named study itself is hosted — that remains an open item (NTRS search endpoint was down this round; retry when it recovers). The remaining unanchored rows are NRE/reliability/capital-cost figures that are management assumptions by nature, not physical measurements.
